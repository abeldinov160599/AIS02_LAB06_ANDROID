...
public class SQLiteFreeIdes extends SQLiteOpenHelper {
    private static final int DATABASE_VERSION = 8; // НОМЕР ВЕРСИИ БАЗЫ ДАННЫХ И ТАБЛИЦ !

    static final String DATABASE_NAME = "free_software"; // Имя базы данных

    static final String TABLE_NAME = "best_free_ides"; // Имя таблицы
    ...
    static final String ASSETS_FILE_NAME = "free_ide.txt"; // Имя файла из ресурсов с данными для БД
    static final String DATA_SEPARATOR = "|"; // Разделитель данных в файле ресурсов с телефонами

    private Context context; // Контекст приложения

    public SQLiteFreeIdes(Context context) {
    ...
    public void addData(SQLiteDatabase db, String name, String site, String platform, String lang) {
        ...
        try {
            ...
            while ((str = in.readLine()) != null) { // Читаем строку из файла
                ...
                if (!strTrim.equals("")) { // Если строка не пустая, то
                    ...
                    addData(db, name, site, platform, lang); // Добавляем название и телефон в базу данных
                }
            }
